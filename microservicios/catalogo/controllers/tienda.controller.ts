...
export class TiendaController {
  ...
  static async getStores(req: Request, res: Response) {
    try {
      ...
      return !tiendas
        ? res.status(404).json({ message: "No se encontraron tiendas" })
        : res.status(200).json({
  ...
  static async getStoreById(req: Request, res: Response) {
    try {
      ...
      return !tienda
        ...
        : res
            .status(200)
            .json({ message: "Tienda obtenida correctamente", data: tienda });
    } catch (error) {
  ...
  static async updateStoreById(req: Request, res: Response) {
    try {
      ...
      const result = await TiendaRepository.actualizarTiendaPorId(
        Number(id),
        tiendaUpdates
      );
      return !result
  ...
  static async deleteStoreById(req: Request, res: Response) {
    try {
      ...
      return !result
        ...
        : res
            .status(200)
            .json({ message: "Tienda eliminada correctamente", data: result });
    } catch (error) {
